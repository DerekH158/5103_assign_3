...
server <- function(input, output, session) {
    ...
    #barplot
    output$beeplot1 <- renderPlot({ # FIXED
        data <- bees3
        if (input$stt != "All") {
            data <- data[data$StateName == input$stt,]
        }
        ...
        #render barplot
        boxplot(`Total production` ~ year, data=data,
                main="Total Production",  #input$totalprod,
                ylab="Total bee production",
                xlab="Year")
    })
    
    #barplot #2
    output$beeplot2 <- renderPlot({ # FIXED using ggplot
        ...
        #secondary barplot
        #    barplot(x = year, y = bees2$priceperlb,
        # main="Price per pound by year",
        # xlab="Year",
        # ylab="Price per lb.",
        # col="blue")
        ggplot(data=data, aes(x = factor(year), y=`Price per lb.`)) +
            geom_boxplot(colour = "blue") +
            labs(title = paste0("Price per pound by ",input$year),
                 x="Year",
                 y=paste0("Price per lb.", input$stt))
    })
